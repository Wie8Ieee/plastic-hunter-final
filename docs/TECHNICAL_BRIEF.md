...
- Run the trained CV pipeline on a GPU-capable deployment target.
- Store per-object detection labels in SQLite for historical type analytics.
- Validate sonar assumptions with real acoustic hardware trials.
- Add authentication and project/team separation for multi-user field deployment.
- Add exportable mission reports for cleanup teams and reviewers.
